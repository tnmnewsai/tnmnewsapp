...
import { exchangeXCode } from "@svt/publishing-x";
...
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(new URL("/platforms?error=X%20did%20not%20return%20an%20auth%20code.", req.url));
  }
  ...
  try {
    ...
    const tokens = await exchangeXCode(credentials, code, redirectUri(), codeVerifier);
    ...
    const response = NextResponse.redirect(new URL("/platforms?connected=X", req.url));
